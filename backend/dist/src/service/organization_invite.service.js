...
async function send_organization_invite(inviteDTO) {
    ...
    const org_ids = await organization_member_repository.get_organization_ids_of_member(user.id);
    const is_user_in_the_organization = org_ids?.find(org_id => org_id === inviteDTO.org_id);
    if (!user)
    ...
    if (inviteExists.length > 0 && inviteExists[0].status === 'pending')
        return { ok: false, error: 'The user already has pending invitation for given organization.' };
    ...
    const [invite] = await organization_invite_repository.send_organization_invite(finalDTO);
    const [receiver] = await organization_invite_repository.get_receiver_id_by_invite_id(invite.id);
...
async function get_organization_invites_of_member(userId) {
    ...
    if (!result.length)
        return { ok: false, message: 'No pending invites.' };
    return { ok: true, data: result };
}
async function answer_organization_invite(answer, invite_id) {
    ...
    if (org.status === 'accepted')
        await organization_member_repository.create_organization_member(org.org_id, org.user_id, org.role);
    return { ok: true };
}
