...
async function get_cards_by_column_id(columnId) {
    return await db
        .select()
        .from(card)
...
async function get_card(card_id) {
...
async function switch_card_positions(dragged_card, dropped_card) {
    await db
        .update(card)
    ...
    await db
        ...
        .where(and(ne(card.id, dragged_card.id), ne(card.id, dropped_card.id), gt(card.position, dropped_card.position), eq(card.column_id, dropped_card.column_id)));
}
