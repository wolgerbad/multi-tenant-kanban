...
async function get_card_comments(card_id: number) {
   return await db.select({ comment: card_comment, user: users }).from(card_comment).where(eq(card_comment.card_id, card_id)).leftJoin(users, eq(users.id, card_comment.sender_id))
}
