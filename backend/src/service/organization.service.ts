...
async function get_organizations_of_member(userId: number) {
  const orgIds =
    await organization_member_repository.get_organization_ids_of_member(userId);
  const orgs =
    await organization_repository.get_organizations_of_member(orgIds);
...
async function create_organization(DTO: {
  ...
  const [organization] = await organization_repository.create_organization({
    organization_title: DTO.organization_title,
    organization_image: DTO.organization_image,
  });
  if (!organization)
