...
async function get_boards_of_organization(orgId: number) {
  ...
  if (!result.length)
    return { ok: false, message: 'No board found on the given organization' };
...
async function get_board_by_id(boardId: number) {
  ...
  if (!result)
    return { ok: false, message: 'No board found with the given id' };
  return { ok: true, data: result };
}

async function create_board(DTO: {
  board_title: string;
  organization_id: number;
}) {
  return await board_repository.create_board(
    DTO.organization_id,
    DTO.board_title
  );
}
