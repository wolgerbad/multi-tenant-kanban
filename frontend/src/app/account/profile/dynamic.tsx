...
export function ProfileForm({ user }: { user: User }) {
  ...
  return (
    <form action={handle_update_profile} className="space-y-4">
      ...
      {error && (
        <p className="text-red-600 text-sm font-medium -mt-3 mb-2">{error}</p>
      )}
...
export function UploadDialog({
  ...
  async function handle_image_upload(formData: FormData) {
    ...
    if (image?.size > 2097151)
      return setError('Image size should be less than 2MB.');
    if (!image?.type.startsWith('image'))
      return setError('Invalid file. Please upload an image.');

    const result = await create_image_url(image);

    if (!result.ok)
      return setError('Error occured while uploading! Try again later.');
      ...
      <DialogContent className="sm:max-w-md">
        <form action={handle_image_upload} className="flex flex-col gap-4">
          ...
          <div className="border-3 border-dashed rounded-lg flex flex-col gap-5 px-8 py-4 justify-center items-center ">
            <div className="flex flex-col gap-1 items-center">
              ...
              <p className="text-sm text-gray-500">
                Please upload an image file that is less than 2MB in size. Other
                file types are not permitted.
              </p>
            </div>
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              className="bg-emerald-400 hover:bg-emerald-500 cursor-pointer"
            >
              Upload Image
            </Button>
          </DialogFooter>
