...
export function CreateNewOrganization({ user_id }: { user_id: number }) {
  ...
  async function handle_create_new_organization(formData: FormData) {
    ...
    if (title?.length < 4)
      return setError('Title must be at least 4 charachter');
    if (image?.name && image?.type?.startsWith('image')) {
      const url_result = await create_image_url(image);
      if (!url_result.ok)
        return setError(
          'Something went wrong with uploading image. Try again later.'
        );
      const res = await upload_image_to_bucket(
        url_result.data.signed_url,
        image
      );
      if (!res.ok)
        return setError(
      ...
      await create_new_organization({
        ...
        organization_image: url_result.data.final_url,
        user_id,
      });
      setIsOpen(false);
      setIsLoading(false);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          className="bg-slate-400/70 hover:bg-slate-400/60 border-none rounded-sm text-white hover:text-white cursor-pointer"
        >
      ...
      <DialogContent className="sm:max-w-sm bg-slate-800 text-white">
        <form
          action={handle_create_new_organization}
        ...
        >
          ...
          <div className="flex flex-col gap-2">
            <div>
              <label htmlFor="name-1" className="block mb-1">
                Organization Title
              </label>
              <input
                id="name-1"
            ...
            <div>
              <label htmlFor="username-1" className="block mb-1">
          ...
          <DialogFooter>
            <DialogClose asChild>
              <Button
                className="bg-transparent hover:bg-slate-900 text-white hover:text-white cursor-pointer"
                variant="outline"
              >
                Cancel
              </Button>
