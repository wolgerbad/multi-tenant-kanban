...
export default function Navbar({session}) {
   return <header className="flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4 text-xs text-slate-300/80">
            ...
            {!session?.ok && <Link href='/login' className="rounded-full bg-slate-100 px-3 py-1 text-slate-900 text-xs font-semibold hover:bg-white transition">
              Sign in
            </Link>}
            {session?.ok && <Link href='/organizations' className="rounded-full bg-slate-100 px-3 py-1 text-slate-900 text-xs font-semibold hover:bg-white transition">
              Go to boards
            </Link>}
          </nav>
        </header>
